...
class Sequencing:
    ...
    def _init_traces(self):
        ...
        if self.mapping_path:
            ...
            for l in h:
                ...
                elif not good_traces:
                    print "No trace found with identifier '%s' : sequence will be ignored" % id
                    continue
                ...
                for trace in good_traces:
                    quality, seq = get_trim('%s/%s%s' % (self.trace_path, subfolder, trace))
                    if seq:
            ...
            for trace_file in trace_files:
                ...
                if not parse:
                    ...
                    quality, seq = get_trim('%s/%s' % (self.trace_path, trace_file))
                    if seq:
    ...
    def analyse(self):
        ...
        for construct in self.constructs:
            ...
            done_nbr += 1
            print '%i%%' % int(100*done_nbr/total_nbr)
        return
...
""" % (valid_nbr, len(self.constructs), len(self.constructs)-valid_nbr, len(self.constructs)))
    ...
    def reorganize(self):
        ...
        for construct in self.constructs:
            ...
            for clone in self[construct].valid:
                os.system ('cp "%s/%s.%s.fas" "%s/%s.%s.fas"' % (output_folder, construct, clone.id, construct_output, construct, clone.id))
                os.system ('cp "%s/%s.%s.html" "%s/%s.%s.html"' % (output_folder, construct, clone.id, construct_output, construct, clone.id))
        ...
        for start_obj in self.sequencing.starts:
            ...
            if isinstance(start, str):
                if 'oFAB'.lower() in start.lower():
                    ...
                    if pos == False:
                        raise IOError('No match found for %s (%s)' % (start, self.id))
        ...
        for stop_obj in self.sequencing.stops:
            ...
            if isinstance(stop, str):
                if 'oFAB'.lower() in stop.lower():
                    ...
                    if pos == False:
                        raise IOError('No match found for %s' % start)
                    stops.append(pos-1)
                else:
...
class Clone:
    def __init__(self, id, construct, location='No location'):
        ...
        self.errors_range = {'substitution':{'nbr':0, 'mem':-1, 'bg':[],'end':[]}, 
                             ...
                             'arrangement':{'nbr':0, 'mem':-1, 'bg':[],'end':[]}}
        self.alns = None
        self.comments = []
        self.names = {}
        self.fasta = ''
    ...
    def write_fasta(self, output_folder):
        self.fasta = '%s/%s.%s.fas' % (output_folder, self.construct.id, self.id)
    ...
    def analyse_aln(self):
        ...
        for aln in self.alns:
            ...
            map = AlnMap(aln)
            if len(aln[0].seq.tostring().replace('-','')) != len(self.construct):
                self.comments.append("Assembled traces do not match reference sequence (assembly size %i vs %i for the reference)" % (len(aln[0].seq.tostring().replace('-','')), len(self.construct)))
                self.errors_range['arrangement']['nbr'] +=1
                self.errors_range['arrangement']['bg'].append(0)
                self.errors_range['arrangement']['end'].append(len(aln[0]))
            else:
                ...
                for type in ('substitution','coverage','insertion','deletion','arrangement'):
    ...
    def write_aln(self, outpout_folder, text=True, html=True):
        if not self.alns:
            print "No alignments for %s.%s" % (self.construct.id, self.id)
            h = open('%s/%s.%s.html' % (output_folder, self.construct.id, self.id), 'w')
...
""" % (self.construct.id, self.id))
        ...
        errors = [[],[]]
        for aln in self.alns:
            updated_ruler = ' '*(self.names['max']+3)
            for seq_index in range(len(aln)):
                to_print_html += self.names[aln[seq_index].id[0]].ljust(self.names['max']) +'   '
                for pos in range(len(aln[0])):
                    for type in ('substitution','insertion','deletion','coverage','arrangement'):
...
def cap3_to_alns(cap3_path, folder, construct, clone):
    ...
    p = subprocess.Popen('%s "%s/%s.%s.fas" %s' % (cap3_path, folder, construct, clone, option), shell=True, stdout=subprocess.PIPE)
...
def get_parameters():
    ...
    o, a = getopt.getopt(sys.argv[1:], 'a:hm:o:p:r:s:t:z:')                            
    opts = {}
    for k,v in o:                                                             
        opts[k] = v
    if opts.has_key('-h'):                                                   
...
seq_check : align sequencing traces against references sequences and check for consistency  
 ...
 seq_check  [-h]  [-p <parse_regexp>] [-a <starts>] [-z <stops>] [-s <primers_file>] [-m <mapping>] -t <input_traces_folder> -r <input_reference_seq> -o <output_folder>
...
if __name__ == '__main__':
    ...
    data.analyse()
    data.reorganize()
    data.summarize_better()
    #print json.dumps(data.summarize_json())
    print 'All set !'
